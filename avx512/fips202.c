...
{
        // TODO: Can be optimized its just a bit of a mess
        int round;
...
{
  // For this case we know the Keccak state is always 25 elements in length
  __m512i zero = _mm512_setzero_si512();
  // Given the known length we can initialize the vector using intrinsics and OpenMP
  #pragma omp parallel sections shared(s, zero)
  {
    #pragma omp section
    _mm512_store_si512(&s, zero);
    #pragma omp section
    _mm512_store_si512((&s + 8), zero);
    #pragma omp section
    _mm512_store_si512((&s + 16), zero);
  }
  s[24] = 0; // Last element is also zero
}
