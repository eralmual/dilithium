...
void polyvec_matrix_expand(matrix* A, const seed* seed) {
  // Get the number of elements per register
  const uint8_t reg_size = REG_BIT_LEN / 32;
  const uint32_t num_elements = A->rows * A->cols;


  for (uint32_t offset = 0; offset < num_elements; offset += reg_size) {
    generate_coeffs(A, seed, offset);
  }
}

void generate_coeffs(matrix* A, const seed* seed, uint32_t offset){}
